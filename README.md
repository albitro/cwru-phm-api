...
```
POST /predict
    │  { signal: [4096], fs: 12000, rpm: 1772 }
    ▼
┌────────────────────────────────────────────────────────┐
│ FastAPI  (api/main.py + routers/predict.py)            │
│                                                        │
│  1. 특징 추출  (api/core/features.py)                   │
│       시간 6 + 주파수 3 + 포락선 5 = 14개 특징           │
│       └ NumPy/SciPy: FFT · Hilbert 포락선 · 대역통과 필터 │
│                     │                                  │
│  2. 분류  (api/core/model.py)                           │
│       RandomForest → Normal / Inner / Outer / Ball     │
│                     │                                  │
│  3. 진단  (api/core/diagnosis.py)                       │
│       포락선 dominant peak vs 이론 결함주파수            │
│       └ api/core/fault_freq.py: BPFO / BPFI / BSF (RPM) │
└───────────────────────────┬────────────────────────────┘
                            ▼
   JSON: prediction · features · diagnosis
         · fault_frequencies · meta
```

세그먼트 길이는 4096 고정, 추론은 밀리초 단위. 모델은 컨테이너 기동 시 1회 로드된다.

## 기술 스택

| 영역 | 선택 | 이유 |
|------|------|------|
| API 프레임워크 | **FastAPI + Uvicorn** | 비동기 서빙 + Pydantic 검증 + OpenAPI(Swagger) 문서 자동 생성 |
| 분류 모델 | **RandomForest (scikit-learn)** | 14개 수작업 특징에 강하고 feature importance로 해석 가능/경량(추론 ms) |
| 신호처리 | **NumPy / SciPy** | FFT / Hilbert 포락선 / Butterworth 대역통과로 시간/주파수/포락선 특징 추출 |
| 입력 검증 | **Pydantic v2** | 4096 샘플 길이, 타입 스키마 강제로 잘못된 요청 조기 차단 |
| 모델 직렬화 | **joblib** | 학습된 RF + 메타(정확도/클래스명)를 번들로 저장·로드 |
| 컨테이너 | **Docker + docker-compose** | 의존성 고정, `make up` 한 줄로 재현 |
| 작업 흐름 | **Makefile** | build/train/up/health/example 등 Docker 명령을 타겟으로 래핑 |
...
```bash
make build                   # Docker 이미지 빌드 (최초 1회)
make up                      # 서버 시작
make example-curl-all        # 합성 예제 4종을 curl로 호출 (한 줄 요약)
```
...
make train                   # -> models/rf_v1.joblib 재생성
